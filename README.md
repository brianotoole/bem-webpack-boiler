...
jQuery is included as dependency, to make it easier to grab DOM elements with ease. You are able to use this within any javascript files by importing jquery into the file, like so:
``` javascript 
import $ from 'jquery'; 
```

#####2. [Lazysizes](https://www.npmjs.com/package/lazysizes)
Lazysizes is similar to Lazyload. It is a self-initializing lazyloader for images and is said to be better for SEO, since lazysizes does not hide images/assets from search engines... No matter what markup pattern you use. This package is included as a dependency. 
...
- [ ] Add production build script: on run-script 'build prod', compile images + min assets